...
import {
  ...
  drawOvalActive,
  drawOvalFilled,
  drawOvalFilledActive,
  drawRectangle,
  drawRectangleActive,
  drawRectangleFilled,
  drawRectangleFilledActive,
  drawLaser,
  drawEraserTail,
  drawText,
...
const DrawDesk = ({
  ...
  }, [allFigures, allFadeFigures, allLaserFigures, allEraserFigures, activeFigureInfo, fadeOpacity, isShiftPressed]);
  ...
  const draw = (allFigures, allFadeFigures, allLaserFigures, allEraserFigures, activeFigureInfo, fadeOpacity, offscreenCanvas) => {
    ...
    allFigures.forEach((figure) => {
      ...
      if (figure.type === 'rectangle') {
        drawRectangle(ctx, figure, updateRainbowColorDeg, isShiftPressed)

        if (activeFigureInfo && figure.id === activeFigureInfo.id) {
          drawRectangleActive(ctx, figure, activeFigureInfo.hoveredDotName, isShiftPressed)
        }
      }
      
      if (figure.type === 'rectangle_filled') {
        drawRectangleFilled(ctx, figure, updateRainbowColorDeg, isShiftPressed)

        if (activeFigureInfo && figure.id === activeFigureInfo.id) {
          drawRectangleFilledActive(ctx, figure, activeFigureInfo.hoveredDotName, isShiftPressed)
        }
      }

      if (figure.type === 'oval') {
        drawOval(ctx, figure, updateRainbowColorDeg, isShiftPressed)

        if (activeFigureInfo && figure.id === activeFigureInfo.id) {
          drawOvalActive(ctx, figure, activeFigureInfo.hoveredDotName, isShiftPressed)
        }
      }

      if (figure.type === 'oval_filled') {
        drawOvalFilled(ctx, figure, updateRainbowColorDeg, isShiftPressed)

        if (activeFigureInfo && figure.id === activeFigureInfo.id) {
          drawOvalFilledActive(ctx, figure, activeFigureInfo.hoveredDotName, isShiftPressed)
        }
      }

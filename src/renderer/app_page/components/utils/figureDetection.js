...
import { dotTextMargin, figureMinScale, dotHoverRadius, interpolateWidthSettings } from '../constants.js'
...
const isOnLine = (x, y, figure) => {
  ...
  const tolerance = baseTolerance + widthInfo.figure_size / 2
...
const isSegmentTouchRectangle = (segmentPoints, figure) => {
  ...
  if (figure.type === 'rectangle_filled') {
    if (isOverRectangle(eraseAtX, eraseAtY, figure)) return true
  } else {
    if (isOnRectangle(eraseAtX, eraseAtY, figure)) return true
  }
...
const isSegmentTouchOval = (segmentPoints, figure) => {
  ...
  if (figure.type === 'oval_filled') {
    if (isOverOval(eraseAtX, eraseAtY, figure)) return true
  } else {
    if (isOnOval(eraseAtX, eraseAtY, figure)) return true
  }
...
export const getDotCoordinates = (figure, dotName) => {
  ...
  if (['rectangle', 'rectangle_filled', 'oval', 'oval_filled'].includes(figure.type)) {

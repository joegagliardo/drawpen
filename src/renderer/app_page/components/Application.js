...
import { FaPaintBrush, FaHighlighter, FaRegSquare, FaRegCircle, FaSquare, FaCircle, FaArrowRight, FaLongArrowAltRight, FaEraser, FaRegTrashAlt, FaAngleLeft, FaAngleRight, FaAngleUp, FaAngleDown, FaChalkboard } from "react-icons/fa";
...
const Application = (settings) => {
  ...
  const handleKeyDown = useCallback((event) => {
    ...
    if (toolbarSlide === 'main-slide') {
      switch (eventCode) {
        ...
        case 'digit8': {
          const nextWidthIndex = Math.max(0, Math.min(widthList.length - 1, Math.round(activeWidthIndex + direction)));
  ...
  useEffect(() => {
    ...
    const debouncedUpdateSettings = debounce(() => {
      invokeSetSettings({
        ...
        tool_bar_direction: toolbarDirection,
      });
    }, 300);
    ...
    if (isDrawing) {
      ...
      if (shapeList.includes(activeTool)) {
        ...
        if (isShiftPressed) {
          ...
          if (['rectangle', 'rectangle_filled', 'oval', 'oval_filled'].includes(currentFigure.type)) {
            const startPoint = currentFigure.points[0];
  ...
  const handleReset = useCallback(() => {
    console.log('Main -> Renderer: Handle Reset - Clearing all figures');
  ...
  const isInitialCollapsed = useRef(true);
  useEffect(() => {
    console.log('Collapse Effect Check:', { toolbarCollapsed, autoDelete });
    if (isInitialCollapsed.current) {
      console.log('Collapse Effect: Skipping initial run');
      isInitialCollapsed.current = false;
      return;
    }

    if (toolbarCollapsed && autoDelete) {
      console.log('Collapse Effect: Triggering handleReset');
      handleReset();
    }
  }, [toolbarCollapsed, handleReset, autoDelete]);

  const handleToggleToolbar = () => {
    console.log('Main -> Renderer: Toggle Toolbar');

    setShowToolbar((prevShowToolbar) => {
      const nextShowToolbar = !prevShowToolbar;

      if (!nextShowToolbar && autoDelete) {
        handleReset();
      }

      return nextShowToolbar;
    });
  };
  ...
  const handleUpdateToolbarPosition = (_, newSettings) => {
    ...
    setToolbarCollapsed(newSettings.tool_bar_collapsed);
  };
